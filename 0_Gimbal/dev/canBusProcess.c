        ...
        (volatile BarrelStatus_canStruct* db, const CANRxFrame* const rxmsg)
{
    ...
    db->chassis_mode        = (uint8_t) (rxmsg->data8[4]);
    chSysUnlock();
}
...
static void can_processEncoderMessage(const CANRxFrame *const rxmsg) {
    switch (rxmsg->SID) {
        case CAN_CHASSIS_SEND_BARREL_ID:
            can_processSendBarrelStatus(&heat_struct, rxmsg);
            break;
        case CAN_FEEDER_FEEDBACK_MSG_ID:
            can_processChassisEncoder(&feeder_encoder, rxmsg);
            break;
        case CAN_PRELOAD_FEEDBACK_MSG_ID:
            can_processChassisEncoder(&preload_encoder, rxmsg);
            break;
...
void can_processInit(void) {
    ...
    memset(&feeder_encoder, 0, sizeof(ChassisEncoder_canStruct));
    memset(&preload_encoder, 0, sizeof(ChassisEncoder_canStruct));
